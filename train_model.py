...
#to avoir error of truncated images
...
def test(model, test_loader, criterion, device, epoch_no, hook):
    ...
    model.eval()
    #setting the debugger hook to EVAL
    hook.set_mode(smd.modes.EVAL) 
    running_loss = 0
    running_corrects = 0
    #we don't need to calculte gradient
    with torch.no_grad(): 
...
def train(model, train_loader, criterion, optimizer, device, epoch_no, hook):
    ...
    model.train()
    # setting the  debugger hook mode to TRAIN
...
def main(args):
    ...
    for epoch_no in range(1, args.epochs +1 ):
        logger.info(f"Epoch {epoch_no} - Training phase.")
        ...
        logger.info(f"Epoch {epoch_no} - Testing phase.")
...
if __name__=='__main__':
    ...
    parser.add_argument( "--weight_decay", type=float, default=1e-2, metavar="WEIGHT-DECAY", help="weight decay coefficient" )
