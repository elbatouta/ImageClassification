...
#this variable is set to solve the problem of truncated images
...
def test(model, test_loader,criterion,device):
    ...
    for inputs, labels in test_loader:
        ...
        #running loss is calculed
        running_loss += loss.item() * inputs.size(0)
        #running corrects is calculed
...
def train(model, train_loader, validation_loader, criterion, optimizer, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch==0:
            break
    print(f"Train set: Average loss: {best_loss}")
    return model
...
def net():
    ...
    '''
    #Resnet50 model is used there!!
   # model = models.resnet30(pretrained=True)
...
def main(args):
    ...
    print("Training...!")
    train(model, train_loader,validation_loader, loss_criterion, optimizer,device,args.epochs)
